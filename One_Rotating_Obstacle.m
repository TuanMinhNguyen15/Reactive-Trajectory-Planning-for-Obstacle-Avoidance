%% 1 Moving Obstacle - Angular Velocity
clear
close all
clc
...
for i = 1:steps
    ...
    pos_dot = Rotate(pos_dot,-w_obs*tstep*(i-1));
    v_obs = double(subs(fw,[x,y,w],[pos_wrt_obs_current,w_obs]));
...
%% 1 Moving Obstacle - Angular Velocity - With Respect To Obstacle - Plot
close all
clc
...
%% 1 Moving Obstacle - Angular Velocity - Plot
figure
...
%% 1 Moving Obstacle - Angular Velocity - Movie
close all
clc
...
for k=1:length(pos)-1
    ...
    plot(ellipse_rotate(:,1)+xc,ellipse_rotate(:,2)+yc);
    axis([-10,10,-10,10])
    hold off
    pause(tstep)
end
